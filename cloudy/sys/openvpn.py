...
def sys_openvpn_docker_install(
    ...
    c.run(f"docker run --rm -v {docker_data}:/etc/openvpn {repo} ovpn_genconfig -u {proto}://{domain}:{port}")
    ...
    # Note: Fabric 2+ does not support interactive prompts natively like Fabric 1.x's settings(prompts=...)
    # If you need to handle prompts, consider using pexpect or ensure 'nopass' is used for automation.
    c.run(cmd)

    c.run(f"docker run -v {docker_data}:/etc/openvpn --name {docker_name} -d -p {port}:1194/{proto} --cap-add=NET_ADMIN {repo}")
