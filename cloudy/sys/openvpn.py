...
def sys_openvpn_docker_install(
    ...
    config_cmd = (
        f"docker run --rm -v {docker_data}:/etc/openvpn {repo} ovpn_genconfig "
        f"-u {proto}://{domain}:{port}"
    )
    c.run(config_cmd)
    ...
    # Note: Fabric 2+ does not support interactive prompts natively like Fabric 1.x
    # If you need to handle prompts, consider using pexpect or ensure 'nopass' for automation.
    c.run(cmd)
...
def sys_openvpn_docker_create_client(
    ...
    if passphrase == "nopass":
        cmd = (
            f"docker run --rm -v {docker_data}:/etc/openvpn -it {repo} easyrsa "
            f"build-client-full {client_name} nopass"
        )
    else:
        cmd = (
            f"docker run --rm -v {docker_data}:/etc/openvpn -it {repo} easyrsa "
            f"build-client-full {client_name}"
        )
    ...
    cmd = (
        f"docker run --rm -v {docker_data}:/etc/openvpn {repo} ovpn_getclient "
        f"{client_name} > /tmp/{client_name}.ovpn"
    )
    c.run(cmd)
