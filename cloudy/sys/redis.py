...
@task
def sys_redis_install(c: Connection) -> None:
...
@task
def sys_redis_configure_memory(c: Connection, memory: int = 0, divider: int = 8) -> None:
    """
...
@task
def sys_redis_configure_interface(c: Connection, interface: str = '0.0.0.0') -> None:
...
@task
def sys_redis_configure_db_file(c: Connection, path: str = '/var/lib/redis', dump: str = 'dump.rdb') -> None:
...
@task
def sys_redis_configure_pass(c: Connection, password: str = '') -> None:
    ...
    sys_etc_git_commit(c, 'Configured redis-server (password set)' if password else 'Configured redis-server (password removed)')
    sys_restart_service(c, 'redis-server')

@task
def sys_redis_config(c: Connection) -> None:
