...
def db_pgis_configure(
    ...
    c.sudo('sudo -u postgres psql -d postgres -c "UPDATE pg_database SET datistemplate=\'false\' WHERE datname=\'template_postgis\';"')
    c.sudo('sudo -u postgres psql -d postgres -c "DROP DATABASE template_postgis;"', warn=True)

    c.sudo('sudo -u postgres createdb -E UTF8 template_postgis')
    c.sudo('sudo -u postgres psql -d template_postgis -c "CREATE EXTENSION postgis;"', warn=True)
    c.sudo('sudo -u postgres psql -d template_postgis -c "CREATE EXTENSION postgis_topology;"', warn=True)
    ...
    c.sudo('sudo -u postgres psql -d template_postgis -c "GRANT ALL ON geometry_columns TO PUBLIC;"')
    c.sudo('sudo -u postgres psql -d template_postgis -c "GRANT ALL ON spatial_ref_sys TO PUBLIC;"')
    c.sudo('sudo -u postgres psql -d template_postgis -c "GRANT ALL ON geography_columns TO PUBLIC;"')
