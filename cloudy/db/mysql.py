...
def db_mysql_create_database(c: Context, root_pass: str, db_name: str) -> None:
    ...
    create_cmd = (
        f'echo "CREATE DATABASE {db_name} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" | '
        f"sudo mysql -u root -p{root_pass}"
    )
    c.sudo(create_cmd)
...
def db_mysql_create_user(c: Context, root_pass: str, user: str, user_pass: str) -> None:
    ...
    create_user_cmd = (
        f"echo \"CREATE USER '{user}'@'localhost' IDENTIFIED BY '{user_pass}';\" | "
        f"sudo mysql -u root -p{root_pass}"
    )
    c.sudo(create_user_cmd)
...
def db_mysql_grant_user(c: Context, root_pass: str, user: str, database: str) -> None:
    ...
    grant_cmd = (
        f"echo \"GRANT ALL PRIVILEGES ON {database}.* TO '{user}'@'localhost';\" | "
        f"sudo mysql -u root -p{root_pass}"
    )
    c.sudo(grant_cmd)
