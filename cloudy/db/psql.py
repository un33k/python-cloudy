...
def db_psql_latest_version(c: Context) -> str:
    ...
    result = c.run(
        'apt-cache search postgresql-client- | grep "postgresql-client-[0-9]"', hide=True, warn=True
    )
...
def db_psql_configure(
    ...
    sed_pattern = (
        f"s/#listen_addresses\\s*=\\s*'\"'\"'localhost'\"'\"'/"
        f"listen_addresses = '\"'\"'{interface},127.0.0.1'\"'\"'/g"
    )
    c.sudo(f"sed -i '{sed_pattern}' {postgresql_conf}")
...
def db_psql_user_password(c: Context, username: str, password: str) -> None:
    ...
    c.sudo(
        ...
        f"\"ALTER USER {username} WITH ENCRYPTED PASSWORD '{escaped_password}';\""
    )
...
def db_psql_create_user(c: Context, username: str, password: str) -> None:
    ...
    c.sudo(
        f"sudo -u postgres psql -c "
        f'"CREATE ROLE {username} WITH NOSUPERUSER NOCREATEDB NOCREATEROLE '
        f"LOGIN ENCRYPTED PASSWORD '{escaped_password}';\""
    )
...
def db_psql_add_gis_extension_to_database(c: Context, dbname: str) -> None:
    ...
    result = c.sudo(
        f'sudo -u postgres psql -d {dbname} -c "CREATE EXTENSION IF NOT EXISTS postgis;"', warn=True
    )
    if result.failed:
        print(
            f"Warning: Failed to add PostGIS extension to database '{dbname}'. "
            f"Extension may not be available."
        )
...
def db_psql_create_gis_database_from_template(c: Context, dbname: str, dbowner: str) -> None:
    ...
    template_check = c.run(
        "sudo -u postgres psql -lqt | cut -d \\| -f 1 | grep -qw template_postgis", warn=True
    )
