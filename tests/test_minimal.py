    ...
    def test_recipe_commands_exist(self):
        ...
        for expected in expected_recipes:
            with self.subTest(recipe=expected):
                self.assertIn(expected, recipe_tasks, f"Expected recipe '{expected}' not found")
    ...
    def test_fabric_task_discovery(self):
        ...
        try:
            ...
            result = subprocess.run(
                ["fab", "-l"], cwd=project_dir, capture_output=True, text=True, timeout=30
            )

            # Check that fab -l completed successfully
            self.assertEqual(result.returncode, 0, f"fab -l failed with error: {result.stderr}")
